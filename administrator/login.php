...
if ($act == "doLogin") {
  ...
  if (trim($uemail) == "" or trim($upassword) == "") {
    ...
    $query = "SELECT id, user_type, fullname, email, permission, image_url, random FROM e4_users WHERE user_type = 'admin' AND password = '%s' AND email = '%s' and publish = 1 ";

    $query = sprintf($query, $database->getEscaped($upassword), $database->getEscaped($uemail));
