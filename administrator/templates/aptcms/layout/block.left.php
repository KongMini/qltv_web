...
<aside class="main-sidebar">
	...
	<section class="sidebar">
		...
		order by region";
				...
				(explode('&', explode('module=', $menu_admin->link)[1])[0] == $module && $module != '') ? $check++ : '';
				foreach ($sub_menus as $sub_menu) {
					(explode('&', explode('module=', $sub_menu->link)[1])[0] == $module && $module != '') ? $check++ : '';
				}
			?>
				<li class="treeview <?php echo ($check > 0) ? ' active ' : '' ?>">
					...
					?>
						<ul class="treeview-menu">
							<?php
							foreach ($sub_menus as $sub_menu) {
								(explode('&', explode('module=', $sub_menu->link)[1])[0] == $module) ? $style = ' active ' : $style = '';
							?>
