...
class Model
{
	...
	static function contact_event()
	{
		...
		$query = "SELECT a.* FROM e4_contacts a WHERE a.type = 'contact-event' ORDER BY a.id desc " . $limit;
		$database->setQuery($query);
		$contacts = $database->loadObjectList();

		$query = "SELECT COUNT(*) total FROM e4_contacts a WHERE a.type = 'contact-event' ";
		$database->setQuery($query);
		$totalRows = $database->loadRow();
		View::contact_event($contacts, $totalRows['total'], $maxRows, $curPg);
	}
}
