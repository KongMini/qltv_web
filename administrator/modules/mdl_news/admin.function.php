...
class Model
{
	...
	{
		$str = '';
		$str .= '<a href ="?module=news&task=news_edit&id=' . $row->id . '" ><i class="fa fa-pencil-square-o" data-toggle="tooltip"  title="Cập nhật thông tin"></i></a>&nbsp;&nbsp;';
	...
	static function news_add()
	{
		...
		if ($_POST["submit"] == "news_add") {
			...
			if ($post_id = $database->insertObject('e4_posts', $row, 'id')) {
				...
				$ariacms->redirect("Tạo mới thành công", "index.php?module=news");
			} else {
	...
	static function news_edit()
	{
		...
		if ($_REQUEST["submit"] == "news_edit") {
			...
			View::news_edit_view($taxonomies, $users, $term_taxonomy_id_list, $news_detail, $news);
		}
	}
