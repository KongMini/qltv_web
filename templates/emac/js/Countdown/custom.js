...
*/
//Countdown Gratiful Card
$( function() {
        var endDate = "May 24, 2022 23:59:59";

        $('.countdown-gratiful.simple').countdown({ date: endDate });

    $('.countdown-gratiful.styled').countdown({
          date: endDate,
    ...
    $('.countdown-gratiful.callback').countdown({
          date: +(new Date) + 10000,
          render: function(data) {
...
//Countdown lounge
$(function () {
    var endDate = "June 07, 2022 23:59:59";

    $('.countdown-lounge.simple').countdown({ date: endDate });

    $('.countdown-lounge.styled').countdown({
        date: endDate,
        render: function (data) {
            $(this.el).html("<div>" + this.leadingZeros(data.days, 2) + " <span>DAYS</span></div><div>" + this.leadingZeros(data.hours, 2) + " <span>HRS</span></div><div>" + this.leadingZeros(data.min, 2) + " <span>MIN</span></div><div>" + this.leadingZeros(data.sec, 2) + " <span>SEC</span></div>");
        }
    ...
    $('.countdown-lounge.callback').countdown({
        date: +(new Date) + 10000,
        render: function (data) {
            $(this.el).text(this.leadingZeros(data.sec, 2) + " sec");
        },
...
//Countdown tailor
$(function () {
    var endDate = "June 14, 2022 23:59:59";

    $('.countdown-tailor.simple').countdown({ date: endDate });

    $('.countdown-tailor.styled').countdown({
        date: endDate,
        render: function (data) {
            $(this.el).html("<div>" + this.leadingZeros(data.days, 2) + " <span>DAYS</span></div><div>" + this.leadingZeros(data.hours, 2) + " <span>HRS</span></div><div>" + this.leadingZeros(data.min, 2) + " <span>MIN</span></div><div>" + this.leadingZeros(data.sec, 2) + " <span>SEC</span></div>");
        }
    ...
    $('.countdown-tailor.callback').countdown({
        date: +(new Date) + 10000,
        render: function (data) {
            $(this.el).text(this.leadingZeros(data.sec, 2) + " sec");
        },
