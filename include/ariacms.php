...
class ariacms
{
	...
	public function __construct()
	{
		...
		$query = "SELECT * FROM e4_web_image WHERE status = 'active' ORDER BY position, region asc";
		$database->setQuery($query);
		$rows = $database->loadObjectList();
		for ($i = 0; $i < count($rows); $i++) {
			$this->web_information->{'image-' . $rows[$i]->position} = $rows[$i]->image;
			$this->web_information->image[$i] = $rows[$i];
		}
	...
	{
		$text = trim(strtolower($this->utf8Convert($text)));
	...
	public function paginationPublic($totalRows, $maxRow, $max_page)
	{
		...
		if ($total_page > 1) {
			if ($current > 1) {
				...
				echo '<a class="jp-previous" href="' . $new_url . '">Previous</a>';
			}
			for ($i = $start_page; $i <= $end_page; $i++) {
				$new_url = str_replace('&&', '&', str_replace('.html', '.html?curPg=' . $i . '&', $url));
				if ($current == $i) {
					echo '<a class="jp-current">' . $i . '</a>';
				} else {
			...
			if ($current < $total_page) {
				...
				echo '<a class="jp-next" href="' . $new_url . '">Next</a>';
			}
		}
	}
	...
	/** Chuẩn hóa lại url cho website frontend */
	public function urlStandardizedMenu($url)
	{
		if ($url) {
			$url = (substr_count($url, 'http') > 0) ? $url : ((substr_count($url, '.') > 0) ? 'http://' . $url : $this->actual_link . $url . ((substr_count($url, '#') > 0) ? '' : '.html'));
		}
